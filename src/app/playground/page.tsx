...
function PlaygroundContent() {
  ...
  const sendMessage = async () => {
    ...
    const responses = [
      ...
      '我理解您的需求。基于您提供的信息，这里是我的建议：\n\n```python\n# 示例代码\ndef solution():\n    return "Hello, World!"\n```\n\n这段代码展示了基本的实现方式。您可以根据实际需求进行调整。',
  ...
  const handleClear = () => {
    ...
    if (confirm('确定清空对话？')) {
      ...
      dispatch(showNotification({ message: '已清空' }));
    }
  };
      ...
      <div className="max-w-[1400px] mx-auto px-5 py-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-text-secondary text-sm">在线测试 API 调用效果</p>
          </div>
          <button onClick={() => setShowCode(!showCode)} className={`btn-secondary text-sm ${showCode ? 'bg-primary/20 border-primary' : ''}`}>
            <i className="fas fa-code mr-2" />{showCode ? '隐藏代码' : '查看代码'}
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="space-y-4">
            <div className="bg-dark/60 border border-border rounded-xl p-5">
              <h3 className="font-semibold mb-4">模型设置</h3>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm text-text-secondary mb-2">选择模型</label>
                  <select className="form-control" value={selectedModel} onChange={e => dispatch(setSelectedModel(e.target.value))}>
                ...
                <div>
                  ...
                  <input type="range" className="w-full accent-primary" min="0" max="2" step="0.1" value={temperature} onChange={e => dispatch(setTemperature(Number(e.target.value)))} />
                </div>
                <div>
                  <div className="flex justify-between text-sm mb-2">
                  ...
                  <input type="range" className="w-full accent-primary" min="100" max="4000" step="100" value={maxTokens} onChange={e => dispatch(setMaxTokens(Number(e.target.value)))} />
                </div>
                <div>
                  <label className="block text-sm text-text-secondary mb-2">System Prompt</label>
                  <textarea className="form-control text-sm" rows={3} value={systemMessage} onChange={e => dispatch(setSystemMessage(e.target.value))} />
                </div>
              </div>
            </div>
            ...
            <button className="btn-danger w-full justify-center" onClick={handleClear}>
              <i className="fas fa-trash mr-2" />清空对话
            </button>
          </div>
            ...
            {showCode ? (
              <div className="bg-dark/60 border border-border rounded-xl overflow-hidden">
                <div className="flex justify-between items-center px-4 py-3 bg-dark-light/50 border-b border-border">
                  <span className="text-sm text-text-secondary">Python 示例代码</span>
                  <button onClick={() => { copyToClipboard(codeExample); dispatch(showNotification({ message: '已复制' })); }} className="text-text-secondary hover:text-primary text-sm">
                    <i className="fas fa-copy mr-1" />复制
                  </button>
                </div>
                <pre className="p-4 overflow-x-auto text-sm"><code className="text-text-primary">{codeExample}</code></pre>
              </div>
            ) : (
              <div className="bg-dark/60 border border-border rounded-xl flex flex-col h-[600px]">
                {/* Messages */}
                <div ref={messagesRef} className="flex-1 overflow-y-auto p-4 space-y-4">
                  {messages.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-text-secondary">
                      <i className="fas fa-comments text-5xl opacity-30 mb-4" />
                      <p className="mb-2">开始与 AI 对话</p>
                      <p className="text-sm">在下方输入消息，按 Enter 发送</p>
                    </div>
                  ) : messages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[80%] rounded-xl px-4 py-3 ${msg.role === 'user' ? 'bg-primary text-white' : 'bg-dark-light'}`}>
                ...
                <div className="p-4 border-t border-border">
                  <div className="flex gap-3">
                    ...
                    />
                    <button className="btn-primary self-end px-6" onClick={sendMessage} disabled={sending}>
